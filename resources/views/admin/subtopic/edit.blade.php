    ...
    <div class="content content-two">
        ...
        <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
            <div class="my-auto mb-2">
                <h3 class="mb-1">Edit Sub Topic</h3>
                <nav>
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('product.list') }}">Sub Topics</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">Edit Sub Topic</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <form action="{{route('sub.topic.update')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body pb-1">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select name="popular_id" class="form-control" id="popular_id">
                                            ...
                                            <option value="{{ $populars->id }}"   {{ isset($getdata) && $getdata->age == $populars->id ? 'selected' : '' }}>{{ $populars->title }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="name" class="form-control" value="{{ $getdata->name }}">
                                    @if ($errors->has('name'))
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="quarters" class="form-control" id="status">
                                        ...
                                        <option value="months 1-3" {{ isset($getdata) && $getdata->quarters == 'months 1-3' ? 'selected' : '' }}>Months 1-3</option>
                                        <option value="months 4-6" {{ isset($getdata) && $getdata->quartersage == 'months 4-6' ? 'selected' : '' }}>Months 4-6</option>
                                        <option value="months 7-9" {{ isset($getdata) && $getdata->quarters == 'months 7-9' ? 'selected' : '' }}>Months 7-9</option>
                                        <option value="months 10-12" {{ isset($getdata) && $getdata->quarters == 'months 10-12' ? 'selected' : '' }}>Months 10-12</option>
                                    </select>
                            ...
                            <div class="text-end mt-3">
                               <a href="{{ route('sub.topic.list') }}" class="btn btn-light me-3">Cancel</a>
                                <button type="submit" class="btn btn-primary">Update Sub Topic</button>
                            </div>
                        </div>
                    </div>
                    <!-- /Sub Topic Information -->
...
function loadTopics(popular_id, selected_topic_id = null) {
    if (popular_id) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (key, value) {
                    let selected = selected_topic_id == value.id ? 'selected' : '';
                    $('#topic').append(`<option value="${value.id}" ${selected}>${value.topic_name}</option>`);
                });
            }
        });
    } else {
...
$(document).ready(function () {
    ...
    const selectedTopicId = '{{ $getdata->topic_name ?? '' }}'; // assuming `topic` holds the selected topic id

    // Load on page load if editing
    if (selectedPopularId) {
        loadTopics(selectedPopularId, selectedTopicId);
    }

    // Reload topics on age change
