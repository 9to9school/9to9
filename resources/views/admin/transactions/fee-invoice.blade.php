...
@section('title', 'Event Transaction List')
  ...
  <div class="content" style="font-family: 'Roboto', sans-serif;">
    ...
    <div class="my-auto mb-2">
        ...
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('event.transaction') }}">Event Transaction</a></li>
    ...
    <?php 
     $data = \App\Models\School::find($feedata->school_id);
    ?>
      <!-- Logo and Header -->
      <div class="flex items-center justify-between mb-6">
        ...
        @php
            ...
            $studentId = $feedata->student_id ?? '000';
        @endphp

        <div class="text-right text-gray-700">
          <p><span class="font-semibold">Invoice #:</span> INV-{{ $currentDate }}-000{{ $studentId }}</p>
          <p><span class="font-semibold">Date:</span> {{ date('d-m-y') }}</p>
        </div>
      </div>
<?php 
     $sdata = \App\Models\Student::find($feedata->student_id);
    ?>
      <!-- Bill To -->
      <div class="mb-4">
        <h2 class="font-semibold text-gray-700 mb-1">Bill To:</h2>
        <p>{{ $sdata->father_name ?? ''}}</p>
        <p>{{ $sdata->address ?? '' }}</p>
        <p>{{ $sdata->city ?? '' }}, {{ $sdata->state ?? '' }} {{ $sdata->pincode ?? '' }}</p>
        <p>Phone: {{ $sdata->phone_number_1 ?? ''}}</p>
      </div>
      ...
      <table class="w-full border-collapse border border-gray-300 mb-6">
        ...
        <tbody>
          <tr>
            <td class="border border-gray-300 px-3 py-2">{{ \App\Models\Student::find($feedata->student_id)?->first_name ?? '' }}</td>
            <td class="border border-gray-300 px-3 py-2 text-right">₹{{ $feedata->paid_amount }}</td>
          </tr>
        </tbody>
      </table>
