    ...
    <div class="content">
      ...
      <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
        <div class="my-auto mb-2">
          <h3 class="page-title mb-1">Milestone List</h3>
          <nav>
            <ol class="breadcrumb mb-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">All Milestone</li>
            </ol>
          </nav>
        </div>
        <div class="d-flex my-xl-auto right-content align-items-center flex-wrap">
          ...
          <div class="mb-2">
            <a href="{{ route('milestone.create') }}" class="btn btn-primary d-flex align-items-center"><i
                class="ti ti-square-rounded-plus me-2"></i>Add Milestone</a>
          </div>
        </div>
      </div>
      <!-- /Page Header -->
          ...
          <div class="custom-datatable-filter table-responsive">
            <table class="table datatable">
              ...
              <tbody>
                ...
                <tr>
                  <td>{{ $milestone->id }}</td>
                  <td>{{ $milestone->popular_id }}</td>
                  <td>{{ $milestone->goal_month }}</td>
                  <td>{{ $milestone->goal_heading }}</td>
                  <td>{{ $milestone->goal_description }}</td>
                  <td>{{ $milestone->goal_breaf }}</td>
                  <td>{{ $milestone->topics_including }}</td>
                  

                  <td>
                    <span class="badge {{ $milestone->status == 1 ? 'badge-success' : 'badge-danger' }}">
                      {{ $milestone->status == 1 ? 'Active' : 'Inactive' }}
                    </span>
                  </td>

                  <td>
                    <a href="{{ route('milestone.edit', $milestone->id) }}" class="btn btn-sm btn-warning">Edit</a>
                    <form action="{{ route('milestone.destroy', $milestone->id) }}" method="POST" style="display:inline;">
                      @csrf
                      @method('DELETE')
