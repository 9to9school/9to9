    ...
    <div class="content">
      ...
      <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
        <div class="my-auto mb-2">
          <h3 class="page-title mb-1">Usps List</h3>
          <nav>
            <ol class="breadcrumb mb-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">All Usps</li>
            </ol>
          </nav>
        </div>
        <div class="d-flex my-xl-auto right-content align-items-center flex-wrap">
          ...
          <div class="mb-2">
            <a href="{{ route('usp.create') }}" class="btn btn-primary d-flex align-items-center"><i
                      class="ti ti-square-rounded-plus me-2"></i>Add Usps</a>
          </div>
        </div>
      </div>
      <!-- /Page Header -->
            ...
            <table class="table">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    @if($usp->image)
                    <img src="{{ asset('public/'.$usp->image) }}" width="50" height="50" alt="USP Image">
                    @else
                    No Image
                    @endif
                  </td>
                  <td>{{ Str::limit($usp->description, 50) }}</td>
                  <td>{{ $usp->button_name }}</td>
                  <td>
                    @if($usp->button_link)
                    <a href="{{ $usp->button_link }}" target="_blank">Buuton Link</a>
                    @else
                    N/A
                    @endif
                  </td>
                  <td>
                    <span class="badge {{ $usp->status ? 'badge-success' : 'badge-danger' }}">
                      {{ $usp->status ? 'Active' : 'Inactive' }}
                    </span>
                  </td>

                  <td>
                    <a href="{{ route('usp.edit', $usp->id) }}" class="btn btn-sm btn-warning">Edit</a>
                    <form action="{{ route('usp.destroy', $usp->id) }}" method="POST" style="display:inline;">
                      @csrf
                      @method('DELETE')
