...
@section('title', 'Quiz Content Management')
@section('content')
                    ...
                    <div class="card-body p-0 py-3">
                        ...
                        <div class="custom-datatable-filter table-responsive">
                            <table class="table datatable">
                                ...
                                <tbody>
                                @foreach($visitBookings as $key => $visit)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $visit->title }}</td>
                                        <td>{{ Str::limit($visit->description, 50) }}</td>
                                        <td>
                                            <span class="badge {{ $visit->status == 1 ? 'badge-success' : 'badge-danger' }}">
                                                {{ $visit->status == 1 ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td>
                                            <a href="{{ url('admin/visit-booking', $visit->id) }}" class="btn btn-sm btn-warning">Edit</a>
                                            <form action="{{ url('admin/life-skills-quiz/'.$visit->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
