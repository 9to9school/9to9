...
<div class="content">
    ...
    <div class="content">
      ...
      <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
        <div class="my-auto mb-2">
          <h3 class="page-title mb-1">WebContent List</h3>
          <nav>
            <ol class="breadcrumb mb-0">
              ...
              <li class="breadcrumb-item active" aria-current="page">All WebContent</li>
            </ol>
          </nav>
        </div>
        <div class="d-flex my-xl-auto right-content align-items-center flex-wrap">
          ...
          <div class="mb-2">
            <a href="{{ route('webcontent.create') }}" class="btn btn-primary d-flex align-items-center"><i
      ...
      <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
          <h4 class="mb-3">WebContent List</h4>
 ...
 @foreach($webcontents as $webcontent)
            <tr>
                ...
                <td>
                    @if ($webcontent->image)
                        <img src="{{ asset($webcontent->image) }}" width="50">
                    @endif
                </td>
                <td>
                    @if ($webcontent->background_image)
                        <img src="{{ asset($webcontent->background_image) }}" width="50">
                    @endif
                </td>
                <td>{{ $webcontent->subheading }}</td>
                <td>{{ $webcontent->heading }}</td>
                <td>{{ $webcontent->title }}</td>
                <td>{{ $webcontent->description }}</td>
                <td>{{ $webcontent->button_name }}</td>
                <td>{{ $webcontent->button_link }}</td>
                <td>{{ $webcontent->button_name2 }}</td>
                <td>{{ $webcontent->button_link2 }}</td>
                <td>
                    <span class="badge {{ $webcontent->status ? 'bg-success' : 'bg-danger' }}">
                        {{ $webcontent->status ? 'Active' : 'Inactive' }}
                    </span>
                </td>
            </tr>
            @endforeach
