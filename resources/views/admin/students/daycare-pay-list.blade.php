...
@section('title', 'Daycare Payment List')
...
<div class="page-wrapper">
    <div class="content">
        ...
        if($role == 'admin'){
          ...
          $creatorderroute = 'daycare.create-order';
          
        }else{
           $dashrout = 'school.dashboard';
          $stulistroute = 'school.student.list';
          $creatorderroute = 'school.daycare.create-order'; 
        }
    ?>
        <!-- Page Header -->
        <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
            <div class="my-auto mb-2">
                <h3 class="page-title mb-1">Daycare payment List</h3>
                <nav>
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Daycare payment List </li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
                <h4 class="mb-3">Daycare payment List</h4>
                            ...
                            <?php $i = 1; ?>
                                @foreach($paylistdata as $data)
                                    <tr>
                                        ...
                                        <td>{{ $data->daycare->name ?? '' }} </td>
                                        <td>{{ $data->hours }} hrs </td>
                                        <td>{{ $data->type }} </td>
                                        <td>₹ {{ $data->fees ?? 0.00}} </td>
                                        <td>{{ $data->date ?? ''}} </td>
                                    </tr>
                                    <?php $i++ ; ?>
                                @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Daycare Fees Pay</h4>
            ...
            <div class="modal-body p-4">              
                ...
                @php
                    $role = Auth::user()->role;
                    $baseUrl = $role === 'admin' ? url('/admin/get-daycare/') : url('/school/get-daycare/');
                @endphp
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="field-6" class="form-label">Daycare</label>
                            <select name="daycare" class="form-select w-100" id="daycare_id" required data-url="{{ $baseUrl }}">
                                <option>Select</option>
                                @foreach($daycare as $data)
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="field-6" class="form-label">Hours</label>
                            <select id="ratio_select" class="form-control" name="hours" required>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label for="field-6" class="form-label">Type</label>
...
$(document).ready(function () {
  ...
  $('#daycare_id').on('change', function () {
    var daycareid = $(this).val();
    var url = $('#daycare_id').data('url') +'/'+ daycareid;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data.forEach(function (item) {
                    $('#ratio_select').append( 
                        `<option value="${item.hours}" data-fee="${item.fee}">${item.hours}</option>`
                    );
                });
                ...
                data.forEach(function (item) {
                    $('#type').append( 
                        `<option value="${item.type}" data-fee="${item.fee}">${item.type}</option>`
                    );
                });
